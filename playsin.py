...
def play(Hz=440, fs=8000, sec=1, vol=50):
    data = [int((15000*(vol/100))*np.sin(2*np.pi*x*Hz/fs)) for x in range(sec*fs)]
...
def loop():
    global loopFlag
    while loopFlag:
        play(slider1.GetValue(), slider2.GetValue(), 2, slider4.GetValue())

def ButtonBind(event):
    global loopFlag
    if event.GetId() == 100:
        threading.Thread(target=play, args=(slider1.GetValue(), slider2.GetValue(), slider3.GetValue(), slider4.GetValue())).start()
...
if __name__=='__main__':
    loopFlag = False
    app = wx.App()
    frame = wx.Frame(None, wx.ID_ANY, 'HSPP', size=(600, 350), style=wx.MINIMIZE_BOX | wx.CAPTION | wx.SYSTEM_MENU | wx.CLOSE_BOX)
    ...
    slider3.SetMax(60)
    slider3.SetValue(0)
    slidertext4 = wx.StaticText(panel, label='vol')
    slider4 = wx.Slider(panel, style=wx.SL_HORIZONTAL|wx.SL_LABELS)
    slider4.SetMin(0)
    slider4.SetMax(100)
    slider4.SetValue(50)
    ...
    layout.Add(slidertext4, flag=wx.GROW)
    layout.Add(slider4, flag=wx.GROW)
